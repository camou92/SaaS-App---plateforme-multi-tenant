package com.amoutech.saas.security;

import com.amoutech.saas.exceptions.UnauthorizedException;
import com.amoutech.saas.properties.JwtProperties;
...
public class JwtTokenService {
    ...
    public void init() {
        try {
            ...
            log.error("Error loading pivate key", e);
    ...
    /**
     * Lit le contenu d'une clé PEM.
     * Priorité : fichier sur le système de fichiers (chemin absolu ou relatif au CWD),
     * fallback sur le classpath si le fichier n'existe pas (utile en dev local sans montage externe).
     */
    private String readKeyContent(final String path) throws Exception {
        final File file = new File(path);
        if (file.exists() && file.isFile()) {
            log.debug("Loading key from filesystem: {}", file.getAbsolutePath());
            return new String(Files.readAllBytes(file.toPath()));
        }

        log.debug("Key not found on filesystem, falling back to classpath: {}", path);
        try (final InputStream is = JwtTokenService.class.getClassLoader()
                .getResourceAsStream(path)) {

            if (is == null) {
                throw new RuntimeException("Key not found (checked filesystem and classpath): " + path);
            }
    ...
    private PrivateKey loadPrivateKey(final String privateKeyPath) throws Exception {
        final String key = readKeyContent(privateKeyPath);
    ...
    private PublicKey loadPublicKey(final String publicKeyPath) throws Exception {
        final String key = readKeyContent(publicKeyPath);
